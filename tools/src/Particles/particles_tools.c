    ...
    if (i > 0 && i < grid->np_tot[dir]-1){
      ...
      if ( delta < -(0.5+eps) || delta > (0.5+eps)){
        print ("! Particles_GetWeights(): delta+1/2 = %12.6e; deltaL-1/2= %12.6e\n",
                delta+0.5, delta-0.5);
        print("PID: %i, x1: %12.6e, x2: %12.6e, x3: %12.6e, DIR: %i\n", p->id, p->coord[0], p->coord[1], p->coord[2], dir);
        QUIT_PLUTO(1);
      }
...
/*   /1* Fast search for uniform grid *1/ */
...
/*     if (xp[dir] <  grid->xl[dir][indx[dir]]) indx[dir]--; /1* Prevent round-off *1/ */
/*     if (xp[dir] >= grid->xr[dir][indx[dir]]) indx[dir]++; /1* Prevent round-off *1/ */
/*   } */
