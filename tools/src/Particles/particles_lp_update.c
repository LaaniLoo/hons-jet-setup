...
{  
  ...
  /* Flag shock cells */
  Particles_LP_FlagShock(d, particle_flag_shock, grid);
  ...
  PARTICLES_LOOP(curNode, d->PHead){
    ...
    #elif PARTICLES_LP_RAISE == YES
    Particles_LP_FixValue(p, d, grid); /* We update our particle velocities, density, pressure, and tracer */

    /* Update the last shocked time if necessary */
    Particles_GetWeights(p, p->cell, w, grid);  

    for (sb=0; sb < PARTICLES_LP_SHK_BINS; sb++) {
        if (Particles_Interpolate(particle_flag_shock[sb], w, p->cell) > 0) {
            p->last_shock_time[sb] = g_time;
        }
    }
    #else
    Particles_GetWeights(p, p->cell, w, grid);  
