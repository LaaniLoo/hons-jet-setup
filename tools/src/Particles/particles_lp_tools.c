 ...
 *********************************************************************** */ 
{
  ...
  pl->density = Particles_Interpolate(d->Vc[RHO], wts, pl->cell);
    ...
    for (ii = 0; ii < NTRACER; ii++) {
        pl->tracer[ii] = Particles_Interpolate (d->Vc[TRC + ii], wts, pl->cell);
    }
#endif
...
/* ********************************************************************* */
void Particles_LP_FlagShock(Data *d, double ****flag2, Grid *grid)
/*!
 ...
 * \param [out] flag2 4D Array of the flagged zones.
 *
 *********************************************************************** */
{
  int  i, j, k, nv;
...
/* --------------------------------------------------------
  ...
  /* -- Compute divergence of velocity -- */
    ...
    /* -----------------------------------------------
      ...
      for (sb = 0; sb < PARTICLES_LP_SHK_BINS; sb++) {
          if (gradp > pow(10, ln_shk_min + dln_shk*sb)*pt_min) {
...
#endif /* (PARTICLES_LP_SPECTRA == YES) || (PARTICLES_LP_RAISE == YES) */
