...
int InjectZone(const double x1, const double x2, const double x3);
...
/*!
 *  Initialize particle shock capturing (only for LAGRANGIAN).
 *  These thresholds are only used whe PARTICLES_LP_RAISE == YES
 * 
 ********************************************************************** */
{
    int sb;

    particle_flag_shock = ARRAY_4D(PARTICLES_LP_SHK_BINS, NX3_TOT, NX2_TOT, NX1_TOT, double);
    ln_shk_min = log10((double)PARTICLES_LP_SHK_THRESH_MIN);
    ln_shk_max = log10((double)PARTICLES_LP_SHK_THRESH_MAX);
    dln_shk = (ln_shk_max - ln_shk_min)/((double)PARTICLES_LP_SHK_BINS-1);

    print("> Particle shock capturing\n");
    print("  %d shock thresholds (%f -> %f)\n", (int)PARTICLES_LP_SHK_BINS, pow(10, ln_shk_min), pow(10, ln_shk_max));

    for (sb = 0; sb < PARTICLES_LP_SHK_BINS; sb++) {
        print("  %f", pow(10, ln_shk_min + dln_shk*sb));
    }
    print("\n");
}
#endif
 ...
 *********************************************************************** */
{
    /* Check whether jet particle injection is enabled */
    if (!Particles_JetEnabled()) return;

#if PARTICLES_INJECT_IN_BATCHES == YES
    Particles_InjectJetParticlesBatches(data, grid);
#else
    Particles_InjectJetParticlesStream(data, grid);
#endif
...
int InjectZone(const double x1, const double x2, const double x3) {
    double th = sph_theta(x1, x2, x3);
    return (sph_radius(x1, x2, x3) <= g_inputParam[JET_INJECTION_HEIGHT]*1.1 && (th < (g_inputParam[JET_OA_PRIMARY] * CONST_PI / 180.0) || th > (CONST_PI - g_inputParam[JET_OA_SECONDARY] * CONST_PI / 180.0)));
}
