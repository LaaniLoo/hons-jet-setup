...
static int GetRightmostIndex(int, double ***, Grid *);
...
/* --------------------------------------
  ...
  n = GetRightmostIndex(dir, pr, grid) + 2*ngh;
...
/* ********************************************************************* */
int GetRightmostIndex (int dir, double ***q, Grid *grid)
/*!
 *
...
#if JD_MODE == JD_PRES
        pres = q[k][j][i];
        pres_env = pressure_at_position(x1[i], x2[j], x3[k], NULL);
        if (pres > 1.1*pres_env) return (i);
#else
