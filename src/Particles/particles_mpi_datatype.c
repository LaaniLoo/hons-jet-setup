 ...
 **************************************************************************** */
{
...
#elif (PARTICLES_TYPE == LAGRANGIAN) && (PARTICLES_LP_SPECTRA == NO) && (PARTICLES_LP_RAISE == NO)
  #define N_ELEMENTS  9
...
#elif (PARTICLES_TYPE == LAGRANGIAN) && (PARTICLES_LP_SPECTRA == NO) && (PARTICLES_LP_RAISE == YES)
  ...
  MPI_Datatype types[N_ELEMENTS] = {MPI_DOUBLE,     /* coord[3]      */
                                    ...
                                    MPI_DOUBLE,     /* density      */
                                    MPI_DOUBLE,     /* pressure     */
                                    MPI_DOUBLE,     /* tracer[NTRACER]     */
                                    MPI_FLOAT,      /* last_shock_time[SHOCK_BINS] */
                                    MPI_FLOAT,      /* tinj         */ 
                                    MPI_FLOAT,      /* color        */ 
                                    MPI_INT,	      /* cell[3]      */
                                    MPI_UINT32_T};  /* id           */

  int blocklengths[N_ELEMENTS]  = {3,3,3,3,1,1,NTRACER,PARTICLES_LP_SHK_BINS,1,1,3,1};
  MPI_Aint offsets[N_ELEMENTS];
  ...
  offsets[i++] = offsetof(Particle, tracer);
  offsets[i++] = offsetof(Particle, last_shock_time);
  offsets[i++] = offsetof(Particle, tinj);
  offsets[i++] = offsetof(Particle, color);
  offsets[i++] = offsetof(Particle, cell);
  offsets[i++] = offsetof(Particle, id);
...
#elif (PARTICLES_TYPE == LAGRANGIAN) && (PARTICLES_LP_SPECTRA == YES) && (PARTICLES_LP_RAISE == NO)
