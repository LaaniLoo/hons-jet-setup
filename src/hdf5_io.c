...
#if COMPRESS_HDF5 == YES
 #define CHUNK_SIZE 75 
#endif

#define DISK_BLOCK_SIZE 1048576
...
/* --------------------------------------------------------------
  ...
  dimstr = 3;
   ...
   // H5Pset_alignment(file_access, 0, DISK_BLOCK_SIZE);
   // H5Pget_cache(file_access, &nelemts, &nslots, &nbytes, &w0);
   /* Increase chunk cache size to 10MBs */
   // nbytes = 10 * 1024 * 1024;
   // H5Pset_cache(file_access, nelemts, nslots, nbytes, w0);
   #if MPI_POSIX == YES
...
/* Define "coords" attribute of group "vars" */
  ...
  for (nd = 0; nd < DIMENSIONS; nd++) {
   ...
   /* Create our dataset properties list */
   plist_id_dataset = H5Pcreate(H5P_DATASET_CREATE);
  #if COMPRESS_HDF5 == YES
   print("\n> HDF5 compression enabled\n");
   /* Set up our chunk size array */
   // for (nd = 0; nd < DIMENSIONS; nd++) chunk_size[nd] = min((CHUNK_SIZE), dimens[nd]);
   for (nd = 0; nd < DIMENSIONS; nd++) chunk_size[nd] = (CHUNK_SIZE);
   /* Our data set must be chunked for compression */
   H5Pset_chunk(plist_id_dataset, rank, chunk_size); /* property list id, number of dimensions, chunk size array */
   /* Enable the shuffle filter for better compression ratios */
   H5Pset_shuffle(plist_id_dataset);
   /* Enable ZLIB / DEFLATE compression, level 1 */
   // H5Pset_deflate(plist_id_dataset, 1); /* property list id, compression level */
  #endif
...
/* ------------------------------------
  ...
  for (nc = 0; nc < 3; nc++) {
    dataset = H5Dcreate(group, cname[nc], H5T_NATIVE_FLOAT, dataspace, plist_id_dataset);
