...
#elif DENSITY_PROFILE == ENV_CONST
    ep->rho_0 = g_inputParam[ENV_RHO_0] / vn.density; //NOTE: not sure why I had to add this line in order to print env_properties
#endif
...
inline double density_at_position(const double x1, const double x2, const double x3, void* bep)
{
  ...
  /* Now we check for the tabulated case */
    ...
    // added ENV_CONST, same behaviour as MAKINO,KING
#if DENSITY_PROFILE == ENV_MAKINO || DENSITY_PROFILE == ENV_KING || DENSITY_PROFILE == ENV_CONST
    AnalyticEnvironmentProperties* ep = (AnalyticEnvironmentProperties*) bep;
...
#elif DENSITY_PROFILE == ENV_CONST
    // ENV_CONST just returns central density
    return ep->rho_0;
...
#else
    /* Otherwise we must have either the King, Makino or constant profiles - these are by nature isothermal, so we calculate
...
inline double potential_at_position(const double x1, const double x2, const double x3, void* bep)
{
    ...
    return phi_0*log(1+(r/ep->r_scaling)*(r/ep->r_scaling));
#elif DENSITY_PROFILE == ENV_CONST
    // ENV_CONST currently doesn't use potential
    return 0; 
#elif DENSITY_PROFILE == ENV_TABULATED
...
inline void velocity_at_position(const double x1, const double x2, const double x3, double *v, void* bep) {
  ...
  return;
#elif DENSITY_PROFILE == ENV_CONST
// Initially 0, use to implement wind velocity
    v[IDIR] = 0.0;
    v[JDIR] = 0.0;
    v[KDIR] = 0.0;
#else
